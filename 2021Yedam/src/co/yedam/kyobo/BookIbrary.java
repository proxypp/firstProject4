...
public class BookIbrary {
	...
	public static void main(String[] args) {
		...
		kyoboLib[2] = new Book("혼자공부하는 ", "홍길동", "우리출판사", 35000);
		...
		while (true) {
			...
			if (menu == 1) {
				...
				// 도서명으로 찾아와서 => 도서명(키)
				// 저자, 출판사, 가격 => 항목변경.
				String search = readStr("수정할 도서명을 선택하세요.");
				String author = readStr("변경할 저자를 입력하세요.[변경값을 입력하지않으려면 엔터]");
				String press = readStr("변경할 출판사를 입력[변경안할려면 엔터]");
				String price = readStr("변경할 금액을 입력하세요.[변경안하려면 엔터]");
